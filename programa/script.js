...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderPlans(durations) {
        ...
        selectedPlanId = firstDuration && firstDuration.plans && firstDuration.plans[0] ? firstDuration.plans[0].id : null;
        ...
        const groups = durations.map((duration, index) => {
            const cards = (duration.plans || []).map((plan, planIndex) => (
                `<button type="button" class="plan-card ${index === 0 && planIndex === 0 ? 'active' : ''}" data-plan-id="${Number(plan.id || 0)}">
        ...
        plansPanel.addEventListener('click', (event) => {
            ...
            if (durationBtn) {
                ...
                plansPanel.querySelectorAll('.plans-group').forEach((group) => {
                    ...
                    if (show) {
                        const first = group.querySelector('.plan-card');
    ...
    async function createRegistrationPix() {
        ...
        const raw = await res.json().catch(() => ({}));
        if (!res.ok || raw.success === false) {
            throw new Error(raw.message || 'Nao foi possivel gerar Pix.');
        }
