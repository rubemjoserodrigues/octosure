...
def main():
    parser = argparse.ArgumentParser(
        description="Teste local BetFast: mostra esporte + liga + nome_evento + url_mae + clones_urls."
    )
    ...
    for page in range(args.max_pages):
        ...
        if found:
            print(f"[ok] evento BetFast encontrado na pagina {page}")
            break

        arbs_count = len(payload.get("arbs") or [])
        print(f"[info] pagina={page} sem BetFast (arbs={arbs_count})")
        if arbs_count == 0:
            break
    ...
    def _is_betfast_event_url(url_text: str, fallback_event_id: str) -> bool:
        ...
        if "/event/" in path or "/live/" in path or "/ao-vivo/" in path or "/sports/" in path:
            return True
        ...
        for key in ("event", "eventid", "match", "game", "e"):
    ...
    if is_viable_final_url(urllib_final) and (not is_tracking_intermediate(urllib_final)):
        need_browser = not _is_betfast_event_url(urllib_final, found.get("bookmaker_event_id", ""))
    ...
    url_mae = normalize_catalog_url(found.get("bookmaker_id"), final_url) or final_url
    ...
    if (not clone_urls) and url_mae:
        clone_urls = [url_mae]

    print("========================================")
    print("casa_mae=BetFast")
